...
Description:
----------------
This file defines the 'SearchStack' component, which serves as the navigation stack for the 'Search' feature in the React Native application. It encapsulates the screens for searching food items and viewing detailed information about a selected item.

SearchStack Component:
---------------------
1. Component Purpose:
   - The 'SearchStack' component facilitates navigation for the 'Search' feature.
   - It uses 'createNativeStackNavigator' from '@react-navigation/native-stack' to create a navigation stack.
   - Two screens are included: 'Searchscreen' for searching and 'Fooddetailscreen' for detailed food item information.

2. Component Structure:
   - Stack Navigator:
     - Utilizes 'createNativeStackNavigator' to define a navigation stack with two screens.
   - Screens:
     - 'Searchscreen': Allows users to search for food items.
     - 'Fooddetailscreen': Displays detailed information about a selected food item.

3. Styles:
   - The component defines styles using 'StyleSheet.create' for a consistent and maintainable appearance.
   - Specific styling for the home button text is included.

4. Navigation Options:
   - Navigation options are configured for each screen to customize the appearance of the navigation header.
   - 'Fooddetailscreen' includes a custom header right button for navigation back to the home screen.

Usage:
-------
- Integrate the 'SearchStack' component into the main navigation container alongside other navigation stacks.

*/
import { View, TextInput, StyleSheet, Pressable,Text, Image, ScrollView } from 'react-native'
import React, { useState } from 'react'
import MuiIcon from 'react-native-vector-icons/MaterialIcons';


const styles=StyleSheet.create({
    ss_search_view_container:{
        flex:1,
        flexDirection:'row',
...
const Searchscreen = ({ navigation }) => {
  ...
  return (
    <ScrollView>
    <View style={{flex:1}}>
      <View style={styles.ss_search_view_container}>
        <TextInput 
          style={styles.ss_textinput_style} 
          placeholder='search by food name'
          value={searchItem}
          onChangeText={setSearchItem}
        />
        <MuiIcon name="search" size={37} style={styles.ss_icon_style} />
      </View>
      <View style={{flexDirection:'row',flexWrap:'wrap'}}>
        {foodArray.map((item,index) => {
          if(item.foodName.toLowerCase().includes(searchItem.toLowerCase())){
            return(     
            <View key={index} style={styles.ss_img_viewstyle}>
              <Pressable 
                style={{gap:10}}
                onPress={() => {
                ...
                <Image 
                  ...
                  style={styles.ss_img}
                />
                <Text style={styles.fds_detailtext}>{item.foodName}</Text>
                <Text style={styles.fds_detailtext}><MuiIcon name="currency-rupee" color={'#2a9d8f'}/>{item.price}</Text>
              </Pressable>
            </View>
          )
          }
